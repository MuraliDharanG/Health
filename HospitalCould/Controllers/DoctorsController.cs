...
namespace HospitalCould.Controllers
{
    public class DoctorsController : Controller
    {
        ...
        {
            var data = ReadPatientRegisterationTable(database()).FindAll().Select(s => s);
            return View(data);
        }
        ...
        {
            var Result = ReadCloudFilesTable(database()).FindAll()
                            .Where(s => s.PatientId == PatientId && s.DoctorId == DoctorId);
            var patienskdetails = ReadSkDetails(database()).FindAll().Where(s => s.ResDocId == DoctorId);
            //ReadSkDetails(database()).FindAll().Where(s => s.ResDocId == docId && s.FileId == fileId).Count()
            var resObject = Result.Join(patienskdetails, a => a.Id.ToString(), b => b.FileId, (a, b)
                => new SkViewModel()
                {
        ...
        public ActionResult OtherRequestForApproved(string status, string RID)
        {
            ...
            {
                return View("Not Approved", "NotApproved");
            }
        }
